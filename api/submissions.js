...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const enrichedProducts = submittedProducts.map(p => {
      const catalog = CATALOG[p.sku] || { name: 'Mobile Phone', brand: 'Generic', category: 'Phones', livePrice: 10000, bestPrice: 8900 };
      return { sku: p.sku, name: catalog.name, category: catalog.category, brand: catalog.brand, livePrice: catalog.livePrice, bestPrice: catalog.bestPrice, promoPrice: parseFloat(p.price), promoStock: parseInt(p.stock), status: 'Pending' };
    });
