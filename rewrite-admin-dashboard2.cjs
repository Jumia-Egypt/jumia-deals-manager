...
import { Plus, Edit2, Save, X, Trash2, Smartphone, Tablet, Headphones, Laptop, Mouse, Gamepad2, Shirt, Microwave, Coffee, Refrigerator, Tv, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="datetime-local" className="border border-slate-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all shadow-sm text-sm" value={toLocalISOString(formData.startDate)} onChange={e => setFormData({...formData, startDate: e.target.value ? new Date(e.target.value).toISOString() : formData.startDate})} />
        </div>
        <div>
          <label className="text-xs font-bold block mb-1">End Date</label>
          <input type="datetime-local" className="border border-slate-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all shadow-sm text-sm" value={toLocalISOString(formData.endDate)} onChange={e => setFormData({...formData, endDate: e.target.value ? new Date(e.target.value).toISOString() : formData.endDate})} />
        </div>
      </div>
