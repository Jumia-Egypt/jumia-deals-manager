...
export default function LiveSkus({ vendorId }: LiveSkusProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const [pR, cR] = await Promise.all([fetch(`/api/products?vendor_id=${vendorId}`), fetch('/api/campaigns')]);
        if (!pR.ok) throw new Error('Failed to load products');
        setProducts(await pR.json() || []);
        if (cR.ok) setCampaigns(((await cR.json()) || []).filter((c: Campaign) => c.status === 'Active'));
    ...
    <div className="w-full h-full flex flex-col bg-slate-50 font-sans p-6">
      ...
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Manage your active products and assign SKUs to campaigns.</p>
        </div>
        {!loading && selectedSkus.size > 0 && (
          <span className="ml-auto text-xs font-semibold text-orange-600 bg-orange-50 border border-orange-200 px-3 py-1.5 rounded-lg">
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3 p-6">
              ...
              <p className="text-xs font-semibold text-slate-500">{products.length === 0 ? 'No products uploaded yet.' : 'No products match your filters.'}</p>
            </div>
          ) : (
            <table className="w-full text-xs whitespace-nowrap min-w-[980px]">
              <colgroup>
              ...
              <thead className="sticky top-0 z-10">
                <tr className="bg-orange-50 border-b border-orange-200 text-orange-600">
                  ...
                  <th className="px-3 py-3 text-center font-semibold tracking-wide">Assign Campaign</th>
                </tr>
              </thead>
                    ...
                    <tr key={p.sku} className={`transition-colors duration-100 ${isSel ? 'bg-orange-50 hover:bg-orange-100/70' : 'bg-white hover:bg-orange-50'}`}>
                      <td className="px-3 py-2.5 text-center">
                        <label className="relative inline-flex items-center cursor-pointer">
                          ...
                          <div className="w-8 h-4 bg-slate-200 rounded-full peer peer-checked:bg-orange-500 peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:border-slate-300 after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:after:border-white shadow-inner" />
                        </label>
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <Dropdown value={assignMap[p.sku] || ''} onChange={val => setAssignMap(prev => ({...prev, [p.sku]: val}))}
                          options={campaigns.map(c => ({ label: c.name, value: c.id }))}
                          placeholder="Select" className="rounded-lg py-1.5 px-2.5" />
                      </td>
                    </tr>
                  );
                })}
